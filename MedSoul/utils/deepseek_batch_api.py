"""DeepSeek Batch API wrapper for large-scale pseudo label generation"""
import os
import json
import time
...
class DeepSeekBatchLabeler:
    """Generate pseudo labels using DeepSeek Batch API (50% cost savings)"""
    ...
    def __init__(
        ...
        """
        Initialize DeepSeek Batch Labeler
        
        Args:
            api_key: DeepSeek API key. If None, reads from DEEPSEEK_API_KEY env variable
            base_url: DeepSeek API base URL
            model_name: DeepSeek model name (default: deepseek-reasoner)
            temperature: Sampling temperature (0.0 for deterministic)
            max_tokens: Maximum tokens in response
            label_names: List of pathology label names
            max_wait_hours: Maximum hours to wait for batch completion (24-336)
...
"""
    ...
    def create_batch_job(self, input_file_id: str) -> str:
        ...
        # Format completion_window: must be "24h" for most providers
    ...
    def batch_extract(
        ...
        try:
            ...
            # Check limits (adjust based on DeepSeek's actual limits)
            if file_size_mb > 100:
                raise Exception(f"File size ({file_size_mb:.2f} MB) exceeds 100 MB limit. Please split data.")
            if num_requests > 50000:
                raise Exception(f"Number of requests ({num_requests}) exceeds 50,000 limit. Please split data.")
            
            # Step 2: Upload file
            print("\n" + "="*70)
            print("STEP 2: Uploading file to DeepSeek")
            print("="*70)
