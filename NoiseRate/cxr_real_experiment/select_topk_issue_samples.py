#!/usr/bin/env python3
"""Select the top-k most suspicious sample-level issues from a cleanlab CSV."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Select top-k suspicious sample issues.")
    ...
    if "pool_row_id" not in df.columns:
        raise ValueError(f"CSV missing pool_row_id column: {in_path}")

    sort_cols = []
    ascending = []
    if "sample_quality_self_confidence" in df.columns:
        sort_cols.append("sample_quality_self_confidence")
        ascending.append(True)
    if "issue_rank_self_confidence" in df.columns:
        sort_cols.append("issue_rank_self_confidence")
        ascending.append(True)
    if "est_issue_entry_count" in df.columns:
    ...
    n_issue = len(ranked)
    if n_issue == 0:
        raise ValueError(f"No issue samples found in {in_path}")
