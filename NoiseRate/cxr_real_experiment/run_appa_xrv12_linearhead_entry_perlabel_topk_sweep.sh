#!/bin/bash
#SBATCH --job-name=cxr_xrv12_eptk
#SBATCH --output=slurm_logs/cxr_xrv12_eptk_%j.log
#SBATCH --error=slurm_logs/cxr_xrv12_eptk_%j.err
...
SELECT_SCRIPT="${PROJECT_DIR}/NoiseRate/cxr_real_experiment/select_topk_issue_entries_per_label.py"
...
RUN_ROOT="${PROJECT_DIR}/NoiseRate/cxr_real_experiment/results_appa_xrv12_linearhead_entry_perlabel_topk/slurm_${SLURM_JOB_ID}"

cd "${PROJECT_DIR}" || exit 1
mkdir -p slurm_logs "${RUN_ROOT}"

echo "======================================================================"
echo "Real CXR AP/PA XRV12 Linear-Head Per-Label Entry Top-k Sweep"
echo "======================================================================"
...
run_one () {
  ...
  echo "[Per-label Top-k Entry] building subset ${tag} from fraction=${frac}"
  python -u "${SELECT_SCRIPT}" \
    --input-csv "${ISSUE_CSV}" \
    --output-csv "${subset_csv}" \
    --top-fraction "${frac}"

  echo "[Per-label Top-k Entry] running ${tag}"
...
run_one "top05_entry_perlabel_fraction" "0.05"
run_one "top10_entry_perlabel_fraction" "0.10"
run_one "top20_entry_perlabel_fraction" "0.20"
...
rows = []
for stage_name in [
    "top05_entry_perlabel_fraction",
    "top10_entry_perlabel_fraction",
    "top20_entry_perlabel_fraction",
]:
...
out_path = run_root / "topk_entry_perlabel_sweep_compare.csv"
