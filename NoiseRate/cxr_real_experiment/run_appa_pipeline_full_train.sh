#!/bin/bash
#SBATCH --job-name=cxr_appa_pipe
#SBATCH --output=slurm_logs/cxr_appa_pipe_%j.log
#SBATCH --error=slurm_logs/cxr_appa_pipe_%j.err
...
SCRIPT_PATH="${PROJECT_DIR}/NoiseRate/cxr_real_experiment/cxr_real_full_train_eval_cleanlab.py"
...
VIEW_ARGS=(AP PA)
RUN_ROOT="${PROJECT_DIR}/NoiseRate/cxr_real_experiment/results_appa_pipeline/slurm_${SLURM_JOB_ID}"
...
run_root = Path("/vol/gpudata/yz3522-llmtest/MRes/NoiseRate/cxr_real_experiment/results_appa_pipeline/slurm_${SLURM_JOB_ID}")
rows = []
...
out_path = run_root / "appa_pipeline_compare.csv"
