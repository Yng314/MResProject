#!/usr/bin/env python3
"""
Train a DenseNet on the full real MIMIC-CXR-JPG train split, evaluate on the
official labeled test set, and then run cleanlab on train predictions from this
same trained model.

This is intentionally separate from the OOF/K-fold cleanlab script. It produces:

- a baseline full-train model checkpoint
- test-set metrics against mimic-cxr-2.1.0-test-set-labeled.csv
- fresh train-set cleanlab issue estimates from the fully-trained model
"""
...
def train_full_model(
    ...
    for epoch_idx in range(epochs):
        ...
        loss_sum = 0.0
        sample_count = 0
        print(f"[baseline-train] epoch {epoch_idx + 1}/{epochs} training...")
        ...
        print(f"[baseline-train] epoch {epoch_idx + 1}/{epochs} train_loss={train_loss:.6f}")
        ...
        print(f"[baseline-train] epoch {epoch_idx + 1}/{epochs} val_loss={val_loss:.6f}")
        ...
        if val_loss < best_val_loss:
            ...
            if no_improve >= early_stopping_patience:
                print(f"[baseline-train] early stopping at epoch {epoch_idx + 1} (best_epoch={best_epoch})")
                break

    if val_loader is not None and recover_best_weights and best_state is not None:
        model.load_state_dict(best_state)
        print(f"[baseline-train] restored best weights from epoch {best_epoch}")
...
def write_report(
    ...
    with open(output_dir / "baseline_run_report.txt", "w", encoding="utf-8") as f:
        f.write("Real CXR Baseline Full-Train + Test Eval + Train Cleanlab\n")
        f.write("=========================================================\n\n")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Full-train baseline on real MIMIC-CXR-JPG with GT test evaluation and train cleanlab.")
    ...
    parser.add_argument("--model-backbone", type=str, default="densenet121_pretrained", choices=["lightcnn", "densenet121_pretrained"])
    ...
    output_dir = Path(args.output_dir) if args.output_dir else SCRIPT_DIR / "results_baseline" / timestamp
    ...
    model = create_model(model_backbone=args.model_backbone, n_labels=len(LABEL_NAMES))
    ...
    train_sample_detail_df, train_entry_detail_df, train_cleanlab_per_label_df, train_cleanlab_summary_df = build_train_cleanlab_outputs(
        rows=train_rows,
        raw_labels=train_raw,
        y_binary=train_binary,
        valid_mask=train_valid,
