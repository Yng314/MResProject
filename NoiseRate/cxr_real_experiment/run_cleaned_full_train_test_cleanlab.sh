#!/bin/bash
#SBATCH --job-name=cxr_real_clean
#SBATCH --output=slurm_logs/cxr_real_clean_%j.log
#SBATCH --error=slurm_logs/cxr_real_clean_%j.err
...
BASELINE_DIR="${BASELINE_DIR:-/vol/gpudata/yz3522-llmtest/MRes/NoiseRate/cxr_real_experiment/results_baseline_es/slurm_234377}"
ISSUE_CSV="${BASELINE_DIR}/train_cleanlab_sample_issues_only.csv"

echo "=============================================="
echo "Real CXR Cleaned Train + Test + Cleanlab"
echo "=============================================="
echo "Job ID: ${SLURM_JOB_ID}"
echo "Node: ${SLURMD_NODENAME}"
echo "Time: $(date)"
echo "Working Directory: $(pwd)"
echo "Baseline Issue CSV: ${ISSUE_CSV}"
echo "=============================================="
...
python -u NoiseRate/cxr_real_experiment/cxr_real_full_train_eval_cleanlab.py \
  ...
  --exclude-sample-csv "${ISSUE_CSV}" \
  --exclude-issue-col est_issue_sample \
  --output-dir /vol/gpudata/yz3522-llmtest/MRes/NoiseRate/cxr_real_experiment/results_cleaned_es/slurm_${SLURM_JOB_ID}
