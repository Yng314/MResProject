#!/bin/bash
#SBATCH --job-name=cxr_xrv12_seed
#SBATCH --output=slurm_logs/cxr_xrv12_seed_%j.log
#SBATCH --error=slurm_logs/cxr_xrv12_seed_%j.err
...
VIEW_ARGS=(AP PA)
SEED="${SEED:-13}"
RUN_ROOT="${PROJECT_DIR}/NoiseRate/cxr_real_experiment/results_appa_xrv12_linearhead_seed_sweep/seed_${SEED}_slurm_${SLURM_JOB_ID}"
...
echo "======================================================================"
echo "Real CXR AP/PA XRV12 Linear-Head Seed Sweep"
echo "======================================================================"
...
TOP05_ENTRY_SUBSET="${RUN_ROOT}/top05_entry_issue_subset.csv"
...
python -u "${SELECT_ENTRY_SCRIPT}" \
  --input-csv "${ENTRY_ISSUE_CSV}" \
  --output-csv "${TOP05_ENTRY_SUBSET}" \
  --top-fraction 0.05
...
echo "[Stage 4/6] top05 entry (global)"
python -u "${MAIN_SCRIPT}" \
  "${common_args[@]}" \
  --exclude-entry-csv "${TOP05_ENTRY_SUBSET}" \
  --exclude-entry-issue-col est_issue_entry \
  --output-dir "${RUN_ROOT}/04_top05_entry_global"

echo "[Stage 5/6] top10 entry (global)"
python -u "${MAIN_SCRIPT}" \
  "${common_args[@]}" \
  --exclude-entry-csv "${TOP10_ENTRY_SUBSET}" \
  --exclude-entry-issue-col est_issue_entry \
  --output-dir "${RUN_ROOT}/05_top10_entry_global"

echo "[Stage 6/6] top10 entry (per-label)"
python -u "${MAIN_SCRIPT}" \
  "${common_args[@]}" \
  --exclude-entry-csv "${TOP10_ENTRY_PERLABEL_SUBSET}" \
  --exclude-entry-issue-col est_issue_entry \
  --output-dir "${RUN_ROOT}/06_top10_entry_perlabel"
...
stage_map = [
    ...
    ("top05_entry_global", "04_top05_entry_global"),
    ("top10_entry_global", "05_top10_entry_global"),
    ("top10_entry_perlabel", "06_top10_entry_perlabel"),
]
...
out_path = run_root / "seed_sweep_compare.csv"
